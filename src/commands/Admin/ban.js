	...
	async callback(bot, interaction, guild, args, settings) {
		...
		if (member.user.id === interaction.user.id) {
			embed = new EmbedBuilder()
				...
				.setDescription(
					bot.translate(settings.Language, "Admin/ban:EMBED_SELF_BAN")
				);
		...
		if (member.user.bot) {
			embed = new EmbedBuilder()
				.setColor(bot.config.colorWrong)
				.setDescription(
					bot.translate(settings.Language, "Admin/ban:EMBED_USER_BOT")
				);
		...
		if (
			...
			embed = new EmbedBuilder()
				...
				.setDescription(
					bot.translate(
						settings.Language,
						"Admin/ban:EMBED_USER_PERMS",
						{
							PERMISSIONS: `${bot.codeBlock(
		...
		if (!res.guilds.includes(interaction.guildId)) {
			...
			embed = new EmbedBuilder()
				...
				.setDescription(
					bot.translate(
						settings.Language,
						"Admin/ban:EMBED_SUCCESSFULL_BAN",
						{
							USER: `${bot.codeBlock(
			...
			embed = new EmbedBuilder()
				...
				.setDescription(
					bot.translate(
						settings.Language,
						"Admin/ban:EMBED_ALREADY_BANNED",
						{
							USER: `${bot.codeBlock(
								member.user.username +
									"#" +
