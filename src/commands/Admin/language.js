					...
					options: [
						{
							name: "language-key",
							description:
								"Set the language on your server. (language keys visble with '/language list')",
							required: true,
							type: 3,
						},
					],
		...
		switch (Sub) {
			case "set":
				...
				if (!obj) {
					embed = new EmbedBuilder()
						...
						.setDescription(
							bot.translate(
								settings.Language,
								"Admin/language:EMBED_INVALID_LANGUAGE"
							)
						);
				...
				embed = new EmbedBuilder()
					...
					.setDescription(
						bot.translate(
							obj.name,
							"Admin/language:EMBED_LANGUAGE_SET",
							{
			...
			case "list":
				...
				for (let i = 0; i < languageList.length; i++) {
					str.push(
						`${bot.codeBlock(languageList[i].name)} - ${
							languageList[i].translation
						}`
					);
				}
				let footer = bot.translate(
					settings.Language,
					"Admin/language:EMBED_AVAILABLE_LANGUAGES_FOOTER"
				);
				embed = new EmbedBuilder()
					...
					.setDescription(
						`**__${bot.translate(
							settings.Language,
							"Admin/language:EMBED_AVAILABLE_LANGUAGES"
						)}__**\n${str.join("\n")}\n\n${bot.translate(
							settings.Language,
							"Admin/language:EMBED_AVAILABLE_LANGUAGES_2",
							{ LINK: bot.config.translationLink }
						)}`
					)
					.setFooter({
						text: footer,
