...
module.exports = class Limit extends Command {
     constructor(bot) {
          super(bot, {
               ...
               options: [{
                    ...
                    options: [{
                         name: "song-amount",
                         description: "The amount of songs a non DJ can have in the queue.",
                         type: 4
                    }]
               }, {
                    name: "time",
                    ...
                    options: [{
                         name: 'minutes',
                         description: 'The max. amount of minutes a song should have.',
                         type: 4,
                         required: true,
                    }, {
                         name: 'seconds',
                         description: 'The max amount of seconds the song should have. (59 is max)',
                         type: 4,
                         required: false,
                    }]
               }, {
     ...
     async callback(bot, interaction, guild, args, settings) {
          const Sub = interaction.options.getSubcommand(["show", "song", "time", "song-reset", "time-reset", "reset"]);
          ...
          switch (Sub) {
               case "show":
                    ...
                    if (settings.SongTimeLimitMS > 0) {
                         ...
                         timeLimit = `${bot.translate(settings.Language, 'Admin/limit:NO_LIMIT_SET')}`
                    }
               ...
               case "reset":
                    ...
                    embed = new MessageEmbed()
                         ...
                         .setDescription(bot.translate(settings.Language, 'Admin/limit:EMBED_LIMIT_RESET', {
                              SONGTIME: `${settings.SongTimeLimitMS = 0 ? 'There is no time limit for non DJs.' : 'There is no time limit on songs anymore.'}`,
                              SONGAMOUNT: `${settings.SongUserLimit = 0 ? 'There is no song limit for non DJs.' : 'There is no song limit per user anymore.'}`
                         }))
