...
module.exports = class Queue extends Command {
     ...
     async callback(bot, interaction, guild, args, settings) {
		...
		const parsedQueueDuration = bot.getduration(player.queue.reduce((prev, curr) => prev + curr.duration, 0) + player.queue.current.duration);
		const songStrings = [];
		for (let i = 0; i < player.queue.length; i++) {
			const song = player.queue[i];
               if (settings.Requester) {
                    songStrings.push(`${i + 1}. ${song.author} - ${song.title} [${bot.getduration(song.duration)}] ~ <@${!song.requester.id ? song.requester : song.requester.id}>\n`);
               } else {
                    songStrings.push(`${i + 1}. ${song.author} - ${song.title} [${bot.getduration(song.duration)}]\n`);
               }
		}
