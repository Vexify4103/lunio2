			...
			case "status":
				...
				try {
					...
					let embed = new EmbedBuilder()
						...
						.setDescription(
							settings.Language,
							"Everyone/premium:EMBED_MANAGE_PREMIUM",
							{
				...
				if (settings.permpremium) {
					embed2.setDescription(
						bot.translate(
							settings.Language,
							"Everyone/premium:EMBED_PERMPREMIUM",
							{
								USERSUBSCRIPTION: `${bot.codeBlock(
									userSubscription
								)}`,
								USESLEFT: usesLeft,
					...
					embed2.setDescription(
						bot.translate(
							settings.Language,
							"Everyone/premium:EMBED_NORMALPREMIUM",
							{
								USERSUBSCRIPTION: `${bot.codeBlock(
									userSubscription
								)}`,
								USESLEFT: `${bot.codeBlock(usesLeft)}`,
								PREMIUMSTATUS: `${bot.codeBlock(
									settings.premium
										? `Promoted`
										: `Not Promoted`
								)}`,
			...
			case "upgrade":
				...
				if (!userSettings.premium) {
					embed = new EmbedBuilder()
						...
						.setDescription(
							bot.translate(
								settings.Language,
								"Everyone/premium:EMBED_PREMIUM_REQUIRED"
							)
						);
				...
				if (userSettings.premiumUses === 0) {
					embed = new EmbedBuilder()
						...
						.setDescription(
							bot.translate(
								settings.Language,
								"Everyone/premium:EMBED_NO_USES_LEFT"
							)
						);
				...
				try {
					...
					embed = new EmbedBuilder()
						...
						.setDescription(
							`Could not find a guild by the ID: ${bot.codeBlock(
								ID
							)}.`
						)
						.setDescription(
							bot.translate(
								settings.Language,
								"Everyone/premium:EMBED_NO_GUILDID",
								{
									ID: `${bot.codeBlock(ID)}`,
								}
							)
				...
				if (server) {
					...
					if (serverSettings.premium || serverSettings.permpremium) {
						embed = new EmbedBuilder()
							...
							.setDescription(
								bot.translate(
									settings.Language,
									"Everyone/premium:EMBED_GUILD_ALREADY_PREMIUM"
								)
							);
					...
					embed = new EmbedBuilder()
						...
						.setDescription(
							bot.translate(
								settings.Language,
								"Everyone/premium:EMBED_SUCCESS_UPGRADE",
								{
									SERVERNAME: `${bot.codeBlock(server.name)}`,
									PREMIUMUSESLEFT: `${bot.codeBlock(
										userSettings.premiumUses - 1
									)}`,
								}
							)
						);
