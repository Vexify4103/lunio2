...
module.exports = class Songinfo extends Command {
     constructor(bot) {
          super(bot, {
               ...
               options: [{
                    name: 'song-number',
                    description: 'Shows the detail of a specific song in the queue.',
                    type: 4,
                    required: false,
               }],
               methods: [{
     ...
     async callback(bot, interaction, guild, args, settings) {
          ...
          if (songNumber > 1) {
               if (songNumber > player.queue.size) songNumber = player.queue.size || 0;
               songNumber - 1;
               let footer;
               if (songNumber === 0) {
                    footer = {
                         text: `[${bot.getduration(player.position)}/${bot.getduration(player.queue.current.duration)}]`
                    }
               } else {
                    footer = {
