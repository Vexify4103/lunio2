...
module.exports.getReadableTime = (ms) => {
	if (!ms || (ms && !isFinite(ms))) {
		throw new TypeError(
			"You need to pass a total number of milliseconds! (That number cannot be greater than Number limits)"
		);
	}
	if (typeof ms !== "number") {
		throw new TypeError(
			`You need to pass a number! Instead received: ${typeof ms}`
		);
	}
...
module.exports.getTimeObject = (ms) => {
	if (!ms || typeof ms !== "number" || !isFinite(ms)) {
		throw new TypeError(
			"Final value is greater than Number can hold or you provided invalid argument."
		);
	}
	const result = {
		years: 0,
	...
	while (result.milliseconds >= 1000) {
		if (result.milliseconds >= 3.154e10) {
			result.years++;
			result.milliseconds -= 3.154e10;
		}
		if (result.milliseconds >= 2.592e9) {
			result.months++;
			result.milliseconds -= 2.592e9;
		}
		if (result.milliseconds >= 8.64e7) {
			result.days++;
			result.milliseconds -= 8.64e7;
		}
		if (result.milliseconds >= 3.6e6) {
			result.hours++;
...
module.exports.getTotalTime = (timeFormat, message) => {
	...
	if (
		...
		message.channel
			.error("time:INCORRECT_DELIMITERS")
