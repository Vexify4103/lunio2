...
module.exports = Object.defineProperties(Message.prototype, {
	...
	getArgs: {
		value: function() {
			...
			args.shift();
			if (this.content.startsWith(`<@!${this.bot.user.id}>`)) args.shift();
	...
	checkEmoji: {
		value: function() {
			if (this.channel.type == 'dm') {
				...
				return this.channel.permissionsFor(this.bot.user).has('USE_EXTERNAL_EMOJIS') ? true : false;
			}
		},
	},
	...
	translate: {
		value: function(key, args) {
			const language = this.bot.translations.get(this.guild?.settings.Language ?? 'en-US');
