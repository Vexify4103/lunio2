...
class Lunio extends Client {
			...
			setInterval(
				() =>
					this.user.setActivity(
						`${this.Activity[j++ % this.Activity.length]}`,
						{
							type: type,
	...
	async loadInteractionGroup(category) {
		try {
			const commands = (
				await readdir("./src/commands/" + category + "/")
			).filter((v, i, a) => a.indexOf(v) === i);
			const arr = [];
			commands.forEach((cmd) => {
				if (
					!this.config.disabledCommands.includes(
						cmd.replace(".js", "")
					)
				) {
					const command =
						new (require(`../commands/${category}${path.sep}${cmd}`))(
							this
						);
	...
	async send(interaction, content) {
		...
		if (this.config.debug)
			this.logger.debug(
				`Interaction: ${interaction.commandName} was ran by ${interaction.user.username}.`
			);
		this.commandsUsed++;
	}
	...
	addEmbed(channelID, embed) {
		...
		if (!this.embedCollection.has(channelID)) {
			...
			this.embedCollection.set(channelID, [
				...this.embedCollection.get(channelID),
				embed,
			]);
		}
	}
