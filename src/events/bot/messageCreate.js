...
module.exports = class Message extends Event {
  ...
  async run(bot, msg) {
    ...
    if (irc && msg.author.bot && msg.author.id !== bot.user.id) {
      if (msg.deletable)
        await msg
          .delete()
          .catch((e) =>
            bot.logger.error(`Error deleting message not from Lunio: ${e}`)
          );
      return;
    }
    ...
    if (
      [`<@${bot.user.id}>`, `<@!${bot.user.id}>`].find((p) => msg.content == p)
    ) {
      if (msg.deletable)
        await msg
          .delete()
          .catch((e) => bot.logger.error(`Error deleting mention message.`));
      ...
      if (settings.CustomChannel) {
        let embed = new EmbedBuilder()
          ...
          .setTitle(title)
          .setDescription(
            bot.translate(settings.Language, "messageCreate:MENTION_DESC_1", {
        ...
        let embed = new EmbedBuilder()
          ...
          .setTitle(title)
          .setDescription(
            bot.translate(settings.Language, "messageCreate:MENTION_DESC_2", {
              SERVERID: `${bot.codeBlock(msg.guild.id)}`,
              SUPPLINK: bot.config.SupportServer.link,
              INVITELINK: bot.config.inviteLink,
            })
    ...
    if (irc) {
      ...
      // IF MUSIC EMBED WAS REPOSTED DONT DELETE IT
      //console.log(msg.id);
      //console.log(settings.mChannelEmbedID);

      if (msg.id === settings.mChannelEmbedID)
        return console.log(settings.mChannelEmbedID);
      ...
      if (
        ...
        return setTimeout(async () => {
          await msg
            .delete()
            .catch((e) =>
              bot.logger.error("Error deleting message from Lunio after time")
            );
      ...
      if (
        ...
        return setTimeout(async () => {
          await msg
            .delete()
            .catch((e) =>
              bot.logger.error("Error deleting visible interaction from Lunio")
            );
        }, bot.config.DeleteTimeout);
      }
        ...
        let embed = new EmbedBuilder()
          ...
          .setDescription(
            bot.translate(
              ...
              {
                PERMISSION: `${bot.codeBlock("MANAGE_MESSAGES")}`,
              }
            )
          );
      ...
      if (!member.voice.channel) {
        let embed = new EmbedBuilder()
          ...
          .setDescription(
            bot.translate(settings.Language, "messageCreate:USER_NOT_VC")
          );
      ...
      if (usersettings.guilds.includes(msg.guild.id)) {
        let embed = new EmbedBuilder()
          .setColor(bot.config.colorOrange)
          .setDescription(
            bot.translate(settings.Language, "messageCreate:USER_BANNED")
          );
      ...
      if (
        ...
        (!msg.guild.members.me.voice.channel ||
          msg.guild.members.me.voice.channel.id !== msg.member.voice.channel.id)
      ) {
        let embed = new EmbedBuilder()
          .setColor(bot.config.colorWrong)
          .setDescription(
            bot.translate(settings.Language, "messageCreate:VC_ALREADY_FULL")
          );
      ...
      if (
        ...
        let embed = new EmbedBuilder()
          .setColor(bot.config.colorWrong)
          .setDescription(
            bot.translate(settings.Language, "messageCreate:MISSING_PERM", {
      ...
      if (!bot.checkDJ(member, settings)) {
        let embed = new EmbedBuilder()
          .setColor(bot.config.colorOrange)
          .setDescription(
            bot.translate(settings.Language, "messageCreate:USER_NO_DJ")
          );
