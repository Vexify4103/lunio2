...
(async () => {
	...
	let string = "Joe waited for the (train) ( ) ";
...
async function loadCommands() {
	const cmdFolders = (await readdir("./src/commands/")).filter(
		(v, i, a) => a.indexOf(v) === i
	);
	...
	cmdFolders.forEach(async (dir) => {
		if (
			bot.config.disabledPlugins.includes(dir) ||
			dir == "command.example.js"
		)
			return;
		const commands = (await readdir("./src/commands/" + dir + "/")).filter(
			(v, i, a) => a.indexOf(v) === i
		);
	...
	evtFolder.forEach(async (folder) => {
		...
		folders.forEach(async (file) => {
			...
			try {
				...
				bot.logger.error(
					`Failed to load Event: ${name} error: ${err.message}`
				);
			}
		});
	});
